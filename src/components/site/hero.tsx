...
export function Hero({ settings: s }: Props) {
  const [slide, setSlide] = useState(0);
  const [word, setWord] = useState(0);

  const rotatingWords = s.heroRotatingWords
    .split(",")
    .map((w) => w.trim())
    .filter(Boolean);

  useEffect(() => {
    const t1 = setInterval(() => setSlide((v) => (v + 1) % HERO_IMAGES.length), 6000);
    const t2 = setInterval(
      () => setWord((w) => (w + 1) % Math.max(rotatingWords.length, 1)),
      2400
    );
    return () => {
      clearInterval(t1);
      ...
      <div className="relative w-full mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-16 sm:pb-20 lg:pb-28 pt-32">
        <div className="max-w-3xl">
          ...
          >
            {s.heroTitle1}
            <br />
            <span className="text-white/70">{s.heroTitle2}</span>
            <br />
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white/80">
                {s.heroSocialProofRating} · {s.heroSocialProofReviews} reviews
              </span>
            </div>
            <div className="h-4 w-px bg-white/20 hidden sm:block" />
            <div className="text-sm text-white/80 hidden sm:block">
              <span className="font-semibold text-white">{s.heroSocialProofBrands}</span> brands
            </div>
            <div className="h-4 w-px bg-white/20 hidden sm:block" />
            <div className="text-sm text-white/80 hidden sm:block">
